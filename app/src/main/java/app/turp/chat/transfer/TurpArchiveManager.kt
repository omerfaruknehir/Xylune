...
const val TURP_CHAT_MIME = "application/vnd.turp.chat"
const val TURP_BACKUP_MIME = "application/vnd.turp.backup"
const val TURP_CHAT_EXTENSION = ".turpchat"
const val TURP_BACKUP_EXTENSION = ".turpbackup"


private const val ENVELOPE_SCHEMA = "turp-archive-envelope-v1"
private const val MANIFEST_SCHEMA = "turp-portable-archive-v1"
...
class TurpArchiveManager(
    ...
    suspend fun writeChatToCache(
        ...
        root.listFiles()?.filter { it.isFile && System.currentTimeMillis() - it.lastModified() > SHARE_CACHE_MAX_AGE_MS }
            ?.forEach(File::delete)
        val file = File(root, "${safeFileName(conversation.title)}-${System.currentTimeMillis()}$TURP_CHAT_EXTENSION")
    ...
    suspend fun writeBackupToCache(
        ...
        root.listFiles()?.filter { it.isFile && System.currentTimeMillis() - it.lastModified() > SHARE_CACHE_MAX_AGE_MS }
            ?.forEach(File::delete)
        val file = File(root, "Turp-backup-${System.currentTimeMillis()}$TURP_BACKUP_EXTENSION")
        try {
            file.outputStream().buffered().use { output ->
                writeArchive(
                    output = output,
    ...
    private fun decodePayloadToTemp(uri: Uri, password: String): DecodedPayload {
        ...
        val temp = File.createTempFile("turp-import-", ".zip", File(context.cacheDir, "previews").apply { mkdirs() })
        try {
