...
object WidgetPinning {
    ...
    fun request(context: Context, source: String, grants: WidgetCapabilityGrants): WidgetPinResult {
        val definition = TurpProgramParser.parse(source, TurpProgramSurface.WIDGET).getOrNull() ?: return WidgetPinResult.INVALID
        ...
        val preview = TurpHomeWidgetProvider.views(context, AppWidgetManager.INVALID_APPWIDGET_ID, source, grants, preview = true)
        val extras = Bundle().apply { putParcelable(AppWidgetManager.EXTRA_APPWIDGET_PREVIEW, preview) }
        return if (manager.requestPinAppWidget(ComponentName(context, TurpHomeWidgetProvider::class.java), extras, callback)) {
...
class WidgetPinReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val definition = TurpProgramParser.parse(pending.source, TurpProgramSurface.WIDGET).getOrNull() ?: return
        ...
        AppWidgetManager.getInstance(context).updateAppWidget(id, TurpHomeWidgetProvider.views(context, id, pending.source, pending.grants))
        if (definition.dataSources.isNotEmpty()) WidgetRefreshScheduler.refreshNow(context, id)
    }
}

class TurpHomeWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, manager: AppWidgetManager, appWidgetIds: IntArray) {
        val storage = WidgetStorage(context)
        appWidgetIds.forEach { id ->
            val source = storage.source(id)
            val grants = storage.grants(id)
            val definition = source?.let { TurpProgramParser.parse(it, TurpProgramSurface.WIDGET).getOrNull() }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val transition = TurpProgramRuntime.apply(actionId, definition, storage.state(id).ifEmpty { definition.state })
    ...
    companion object {
        internal fun views(
            ...
            return RemoteViews(context.packageName, R.layout.turp_home_widget_program).apply {
                ...
                if (hasRefresh && !preview && id != AppWidgetManager.INVALID_APPWIDGET_ID) {
                    val refreshIntent = Intent(context, TurpHomeWidgetProvider::class.java)
                ...
                ACTION_IDS.forEachIndexed { index, viewId ->
                    ...
                    if (action != null) {
                        ...
                        if (!preview && id != AppWidgetManager.INVALID_APPWIDGET_ID) {
                            val intent = Intent(context, TurpHomeWidgetProvider::class.java)
        ...
        private fun emptyViews(context: Context) = RemoteViews(context.packageName, R.layout.turp_home_widget_program).apply {
            setTextViewText(R.id.widget_title, context.getString(R.string.app_name))
            setTextViewText(R.id.widget_subtitle, "Add an turp-widget/1 program from a conversation")
            setTextViewText(R.id.widget_status, "Create and add a configured widget from a conversation")
            setImageViewResource(R.id.widget_program_image, R.drawable.ic_turp_mark)
        ...
        private fun visibleActions(definition: TurpProgramDefinition, state: Map<String, String>): List<WidgetVisibleAction> {
            ...
            fun walk(node: TurpProgramNode) {
                ...
                if (node.type == "button" && node.action.isNotBlank()) {
                    values += WidgetVisibleAction(TurpProgramRuntime.render(node.label, state).take(32), node.action)
                }
                if (node.type == "input" && node.action.isNotBlank()) {
                    values += WidgetVisibleAction(TurpProgramRuntime.render(node.label.ifBlank { node.value }, state).take(32), node.action)
                }
                if (node.type == "toggle" && node.action.isNotBlank()) {
                    val label = TurpProgramRuntime.render(node.label.ifBlank { node.value }, state)
                    val stateLabel = if (TurpProgramRuntime.truthy(state[node.value])) "On" else "Off"
                    values += WidgetVisibleAction("$label · $stateLabel".take(32), node.action)
                }
                if (node.type == "choice") {
                    node.options.filter { it.action.isNotBlank() }.forEach { option ->
                        values += WidgetVisibleAction(TurpProgramRuntime.render(option.label, state).take(32), option.action)
                    }
                }
                if (node.type == "list") {
                    node.items.filter { it.action.isNotBlank() }.forEach { item ->
                        values += WidgetVisibleAction(TurpProgramRuntime.render(item.label, state).take(32), item.action)
                    }
                }
...
internal fun grantsSatisfy(definition: TurpProgramDefinition, grants: WidgetCapabilityGrants): Boolean =
...
private const val EXTRA_TOKEN = "turp_widget_token_v2"
private const val EXTRA_ACTION_ID = "turp_widget_action_id"
internal const val EXTRA_WIDGET_ROUTE = "turp_widget_route"
private const val ACTION_PROGRAM_ACTION = "app.turp.chat.widget.PROGRAM_ACTION_V2"
private const val ACTION_REFRESH = "app.turp.chat.widget.REFRESH_V2"
