...
import app.turp.chat.update.InstalledReleaseNotesState
import app.turp.chat.update.RepositoryRelease
...
@Composable
fun TurpApp(viewModel: ChatViewModel, activity: Activity) {
    ...
    val repositoryUpdates = remember(activity) {
        (activity.application as TurpApplication).container.repositoryUpdates
    }
    ...
    val performanceMonitor = remember(activity) { TurpPerformanceMonitor(activity) }
    ...
    SideEffect {
        TurpRenderProfiler.setScreen(screen.name)
...
private fun PerformanceOverlayHost(
    monitor: TurpPerformanceMonitor,
    settings: DeveloperSettings,
    modifier: Modifier = Modifier,
) {
    // Snapshot updates must not recompose TurpApp, the navigation host, drawer,
