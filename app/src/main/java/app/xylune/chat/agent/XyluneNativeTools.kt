...
object XyluneNativeTools {
    ...
    fun definitions(conversation: ConversationEntity, memoryEnabled: Boolean = false): List<NativeToolDefinition> = buildList {
        ...
        if (conversation.webSearchEnabled) {
            ...
            add(tool(
                name = "http_request",
                description = "Call a public HTTPS text/JSON/XML API with an explicit HTTP method, safe non-secret headers, and an optional body. Use this for APIs rather than ordinary webpage reading. GET/HEAD and read-only POST requests may declare effect=read; PUT/PATCH/DELETE and any state-changing POST must declare effect=write and confirmed=true only after the user explicitly confirms that external write in a separate message. Authentication, cookies, API-key, token, and other secret-bearing headers are intentionally rejected because tool arguments are persisted in Xylune diagnostics. Local/private destinations and unsafe redirects are blocked.",
                properties = """"url":{"type":"string","minLength":8},"method":{"type":"string","enum":["GET","HEAD","POST","PUT","PATCH","DELETE"]},"headers":{"type":"object","maxProperties":32,"additionalProperties":{"type":"string"}},"body":{"type":"string","maxLength":256000},"contentType":{"type":"string","maxLength":200},"effect":{"type":"string","enum":["read","write"]},"confirmed":{"type":"boolean"},"maxResponseBytes":{"type":"integer","minimum":1024,"maximum":2000000}""",
                required = listOf("url", "method", "effect"),
            ))
            add(tool(
                name = "graphql_request",
                description = "Call a public HTTPS GraphQL endpoint. Supply the GraphQL document and optional variables as a JSON object; Xylune builds the POST envelope. Queries normally use effect=read. GraphQL mutations must use effect=write and confirmed=true only after the user explicitly confirms the external write in a separate message. Safe non-secret headers are supported, while authentication/cookie/API-key/token headers are rejected until Xylune has a credential-vault reference mechanism.",
                properties = """"url":{"type":"string","minLength":8},"query":{"type":"string","minLength":1,"maxLength":200000},"variables":{"type":"object"},"operationName":{"type":"string","maxLength":200},"headers":{"type":"object","maxProperties":32,"additionalProperties":{"type":"string"}},"effect":{"type":"string","enum":["read","write"]},"confirmed":{"type":"boolean"},"maxResponseBytes":{"type":"integer","minimum":1024,"maximum":2000000}""",
                required = listOf("url", "query", "effect"),
            ))
            add(tool(
                name = "feed_read",
    ...
    fun request(call: NativeToolCall): AgentToolRequest {
        ...
        return when (call.name.lowercase()) {
            ...
            "http_request" -> AgentToolRequest(type = "http_request", url = string("url"), method = string("method"), headers = stringMap("headers"), body = string("body"), contentType = string("contentType"), effect = string("effect"), confirmed = bool("confirmed"), maxResponseBytes = int("maxResponseBytes"))
            "graphql_request" -> AgentToolRequest(type = "graphql_request", url = string("url"), headers = stringMap("headers"), effect = string("effect"), confirmed = bool("confirmed"), maxResponseBytes = int("maxResponseBytes"), graphqlQuery = string("query"), graphqlVariablesJson = objectJson("variables"), graphqlOperationName = string("operationName"))
