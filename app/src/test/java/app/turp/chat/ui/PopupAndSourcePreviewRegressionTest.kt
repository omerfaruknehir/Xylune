...
class PopupAndSourcePreviewRegressionTest {
    ...
    fun `modal Back is keyboard first and tap away waits for release`() {
        val source = repositoryFile("app/src/main/java/app/turp/chat/ui/ReleaseDismissPopup.kt").readText()
        val dialog = source.substringAfter("fun TurpAlertDialog(")
    ...
    fun `source preview uses visible platform dismissable popup`() {
        val sourceBar = repositoryFile("app/src/main/java/app/turp/chat/ui/SourceReferenceBar.kt").readText()
        val linkPreview = repositoryFile("app/src/main/java/app/turp/chat/ui/LinkPreview.kt").readText()
