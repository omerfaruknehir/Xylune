...
class AppVersionAndProviderSheetRegressionTest {
    ...
    fun `runtime version comes from installed package metadata`() {
        val version = repositoryFile("app/src/main/java/app/turp/chat/AppVersion.kt").readText()
        val updates = repositoryFile("app/src/main/java/app/turp/chat/update/RepositoryUpdateManager.kt").readText()
        val settings = repositoryFile("app/src/main/java/app/turp/chat/ui/SettingsScreen.kt").readText()
        val archive = repositoryFile("app/src/main/java/app/turp/chat/transfer/TurpArchiveManager.kt").readText()
        val contextAssembler = repositoryFile("app/src/main/java/app/turp/chat/chat/ContextAssembler.kt").readText()
        val generationWorker = repositoryFile("app/src/main/java/app/turp/chat/generation/GenerationWorker.kt").readText()
        ...
        assertTrue(updates.contains("Turp/\${installedVersion.versionName}"))
        ...
        assertTrue(contextAssembler.contains("Turp app version: \$appVersion (installed Android package version)"))
    ...
    fun `add provider is an adaptive ime safe sheet`() {
        ...
        assertFalse(dialog.contains("TurpAlertDialog("))
    }
}
