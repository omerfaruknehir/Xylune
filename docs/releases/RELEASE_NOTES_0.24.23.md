...
- Fixed Xylune reporting or reasoning from the wrong app version by reading the version name and code from Android's actually installed package metadata, with generated build constants only as a fallback.
...
- The AI runtime context now receives the actual installed Xylune app version and explicitly distinguishes it from the separate core-prompt revision, preventing the prompt revision from being mistaken for the app version.
